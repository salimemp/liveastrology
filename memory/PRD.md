...
- Seed 5 SEO blog articles into the Articles CMS (Moon in Scorpio, Sun vs Moon vs Rising, "Free birth chart calculator: no signup", "How to read your birth chart", "What is a birth chart? Beginner's guide").
- Build "VS / Alternatives" landing pages — `/vs/nebula`, `/vs/co-star`, `/free-no-signup` — targeting high‑intent comparison queries.
- Enrich JSON‑LD: add `HowTo` schema on calculator pages, `Article` schema on blog posts (already partial via breadcrumb).
- Compatibility calculator viral loop ("Check your compatibility with your partner" — already exists at `/synastry`; add share buttons + social cards to results).

### P2 — Marketing audit Phase 3 (strategic)
